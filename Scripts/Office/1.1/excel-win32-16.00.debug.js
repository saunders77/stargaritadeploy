...
OSF.DDA.Context = function OSF_DDA_Context(officeAppContext, document, license, appOM, getOfficeTheme, ui) {
    ...
    if (statusNumber) {
        switch (statusNumber) {
            case -2146828218:
                ...
                break;
            case -2147467259:
                status = OSF.DDA.ErrorCodeManager.errorCodes.ooeActivityLimitReached;
                break;
            case -2146828283:
...
OSF.InitializationHelper.prototype.prepareApiSurface = function OSF_InitializationHelper$prepareApiSurfaceAndInitialize(appContext) {
    ...
    if (appContext.get_isDialog()) {
        if (OSF.DDA.UI.ChildUI) {
            ui = new OSF.DDA.UI.ChildUI();
        }
    }
    else {
        if (OSF.DDA.UI.ParentUI) {
            ui = new OSF.DDA.UI.ParentUI();
        }
    }
    OSF._OfficeAppFactory.setContext(new OSF.DDA.Context(appContext, appContext.doc, license, null, getOfficeThemeHandler, ui));
